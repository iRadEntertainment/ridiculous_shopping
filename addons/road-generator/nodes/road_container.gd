...
func validate_edges(autofix: bool = false) -> bool:
	...
	for _idx in range(len(edge_rp_locals)):
		...
		if not is_inside_tree() or not is_instance_valid(this_pt):
			is_valid = false
...
func rebuild_segments(clear_existing := false):
	...
	for obj in get_roadpoints():
		...
		if pt.prior_pt_init:
			...
			if not is_inside_tree() or not is_instance_valid(prior_pt) or not prior_pt.has_method("is_road_point"):
				prior_pt = null
		if pt.next_pt_init:
			next_pt = pt.get_node_or_null(pt.next_pt_init)
			if not is_inside_tree() or not is_instance_valid(next_pt) or not next_pt.has_method("is_road_point"):
				next_pt = null
...
func on_point_update(point:RoadPoint, low_poly:bool) -> void:
	if not _auto_refresh:
		_needs_refresh = true
		return
	elif not is_inside_tree() or not is_instance_valid(point):
		return
