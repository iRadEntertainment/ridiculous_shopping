...
func should_add_mesh() -> bool:
	...
	var par = get_parent()
	if not is_inside_tree() or not is_instance_valid(par) or not par is RoadPoint:
...
func check_rebuild() -> bool:
	...
	if not is_inside_tree() or not is_instance_valid(container):
		return false
	if not is_inside_tree() or not is_instance_valid(start_point) or not is_instance_valid(end_point):
		return false
...
func generate_lane_segments(_debug: bool = false) -> bool:
	...
	for this_match in _matched_lanes:
		...
		if not is_inside_tree() or not is_instance_valid(ln_child) or not ln_child is RoadLane:
...
func get_lanes() -> Array:
	...
	for ch in _par.get_children():
		if not is_inside_tree() or not is_instance_valid(ch):
			continue
...
func _build_geo():
	if not is_inside_tree() or not is_instance_valid(road_mesh):
		return
