...
func assign_lane(new_lane:RoadLane):
	if not is_inside_tree() or not is_instance_valid(new_lane):
...
func assign_manager() -> int:
	...
	if road_manager_path:
		...
		if not is_inside_tree() or not is_instance_valid(_target_manager):
...
func find_nearest_lane(pos = null) -> RoadLane:
	if not is_inside_tree() or not is_instance_valid(actor) or not is_instance_valid(road_manager):
		return null
	if pos == null:
...
func change_lane(direction: int) -> int:
	...
	if not is_inside_tree() or not is_instance_valid(_new_lane):
