...
func _set_lanes(values):
	lanes = values
	if not is_inside_tree() or not is_instance_valid(container) or not container.is_inside_tree():
...
func get_last_rp(direction: int):
	...
	while _next_itr_point != self:  # Exit cond for a full circle around
		if first_loop:
			...
			if not is_inside_tree() or not is_instance_valid(_next_itr_point) or not _next_itr_point.has_method("is_road_point"):
				return self
