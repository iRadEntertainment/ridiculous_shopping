...
func get_manager_from_selection(): # -> Optional[RoadManager]
	...
	if not is_inside_tree() or not is_instance_valid(selected_node):
...
func get_nearest_road_point(camera: Camera3D, mouse_pos: Vector2) -> RoadPoint:
	...
	if not collider.name.begins_with("road_mesh_col"):
		...
		for idx in len(par_rc.edge_rp_locals):
			...
			if not is_inside_tree() or not is_instance_valid(nearest_point) or edge_dist < nearest_dist:
				nearest_point = edge
				nearest_dist = edge_dist
		if not is_inside_tree() or not is_instance_valid(nearest_point):
			return null
...
func get_nearest_edge_road_point(container: RoadContainer, camera: Camera3D, mouse_pos: Vector2):
	...
	for pth in container.edge_rp_locals:
		...
		if not is_inside_tree() or not is_instance_valid(rp):
			continue
...
func _instance_custom_roadcontainer(path: String) -> void:
	...
	if not is_inside_tree() or not is_instance_valid(t_manager):
...
func _create_road_container_do(t_manager: RoadManager, init_sel: Node) -> void:
	...
	if not is_inside_tree() or not is_instance_valid(t_manager) or not t_manager is RoadManager:
...
func _add_next_rp_on_click(pos: Vector3, nrm: Vector3, selection: Node, auto_connect_rp=null) -> void:
	...
	if not is_inside_tree() or not is_instance_valid(selection):
...
func _delete_rp_on_click(selection: Node):
	...
	if not selection is RoadPoint:
		...
		return
	elif not is_inside_tree() or not is_instance_valid(selection):
		push_error("Invalid selection selection, not valid node")
		return
...
func _create_roadpoint_pressed() -> void:
	var undo_redo = get_undo_redo()
	var t_container = get_container_from_selection()
	if not is_inside_tree() or not is_instance_valid(t_container):
		push_error("Invalid selection context")
		return
...
func _create_roadpoint_do(t_container: RoadContainer):
	...
	if not is_inside_tree() or not is_instance_valid(t_container) or not t_container is RoadContainer:
...
func _create_lane_pressed() -> void:
	...
	if not is_inside_tree() or not is_instance_valid(target_parent):
